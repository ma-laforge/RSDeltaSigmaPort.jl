...
#-------------------------------------------------------------------------------
#Renamed to .jlx extension to skip regression tests.
#=IMPORTANT
...
MALaforge: Started porting code, but stopped when I realized I need firpm().
=#
...
=#
	...
	for fp1 in [.03, .035, .025, .040, .020, .045, .015, .05]
		...
		if !failed
			...
			complexity = size(vcat(f1.csd),2) + (2*n1-1)*(n2+size(vcat(f2.csd),2)-1)
			msg = ""
	...
	if isinf(lowest_complexity)
		@warn("designHBF6: Unable to meet the design requirements.")
	else
